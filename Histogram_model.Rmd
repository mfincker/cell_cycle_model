...
theoretical_count <- function(bin, start_bin, B, C, tau, total_n, time_per_channel) {
  ...
  if (bin < start_bin | bin > 2 * start_bin) {
    ...
    n_theo <- total_n * pop_percent(0, B, tau)
  } else if (bin == 2 * start_bin) {
    n_theo <- total_n * pop_percent(B + C, tau, tau)
  } else {
    n_theo <- total_n * pop_percent(B + time_per_channel * (bin - start_bin),
...
DNAhisto_currentGeneration <- 
  ...
  histo <-
    histo %>%
    mutate(n_theo = map_dbl(bin, 
                            ~theoretical_count(., start_bin, B, C, tau, total_n, time_per_channel))
    )
  ...
  histo <-
    ...
    mutate(std_dev = std_dev + std_dev_var * row_number(),
           n_model = map2(bin, std_dev, 
                          function(x, y) {histo$n_theo * dnorm(histo$bin, x, y) %>% 
                                            as_tibble}) %>% 
                     bind_cols() %>% 
                     summarise_all(sum) %>% 
  ...
  deviation <- 
    histo %>% 
    mutate(dif = (sqrt(n) - sqrt(n_model))^2) %>% 
    filter(bin > 0.5 * c1_peak) %>% # ignore the debris / cells without full DNA
    transmute(dif = dif) %>% 
    sum() %>% 
    sqrt(.) / 255
  ...
  if (plot == TRUE) {
    ...
    p <- histo %>% 
      ggplot(aes(x = bin / c1_peak)) +
      geom_line(aes(y = n), color = "red") +
      geom_line(aes(y = n_model), color = "blue") +
      annotate("text", x = 2.3, y = max(0.9 * histo$n), 
...
DNAhisto_currentGeneration(curr_histo,
                           B = best_fit$B, 
                           C = best_fit$C, 
                           tau = tau, 
